    ...
    if (shouldUseOracle()) {
      await OracleService.insert(
        SQL_QUERIES.EMPLOYEE_ALLOWANCE.INSERT,
        {
          ...
          startDate: newAllowance.startDate ? new Date(newAllowance.startDate) : null,
          endDate: newAllowance.endDate ? new Date(newAllowance.endDate) : null,
    ...
    if (shouldUseOracle()) {
      const affected = await OracleService.update(
        SQL_QUERIES.EMPLOYEE_ALLOWANCE.UPDATE,
        {
          ...
          startDate: updatedAllowance.startDate ? new Date(updatedAllowance.startDate) : null,
          endDate: updatedAllowance.endDate ? new Date(updatedAllowance.endDate) : null,
          description: updatedAllowance.description ?? null,
          status: updatedAllowance.status,
        },
      );
