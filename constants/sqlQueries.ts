  ...
  EMPLOYEE_ALLOWANCE: {
    SELECT_ALL: `
      SELECT
          ...
          ea.AMOUNT AS "amount",
          ea.START_DATE AS "startDate",
          ea.END_DATE AS "endDate",
          ea.DESCRIPTION AS "description",
          ea.STATUS AS "status"
       FROM EMPLOYEE_ALLOWANCE ea
       INNER JOIN EMPLOYEE e ON ea.EMP_ID = e.ID
       ORDER BY ea.START_DATE DESC NULLS LAST
    `,
    
    INSERT: `
      INSERT INTO EMPLOYEE_ALLOWANCE (
          ID, EMP_ID, ALLOWANCE_TYPE, AMOUNT, START_DATE,
          END_DATE, DESCRIPTION, STATUS, CREATED_BY, CREATED_AT
       ) VALUES (
          :id,
          (SELECT ID FROM EMPLOYEE WHERE CODE = :empCode),
          :allowanceType, :amount, :startDate,
          :endDate, :description, :status, 'system', SYSTIMESTAMP
       )
    `,
      ...
      SELECT
        ...
        NVL((
          ...
          WHERE ea.EMP_ID = e.ID
            ...
            AND ((:yearNum * 12 + :monthNum) <= (EXTRACT(YEAR FROM ea.END_DATE) * 12 + EXTRACT(MONTH FROM ea.END_DATE)) OR ea.END_DATE IS NULL)
        ), 0) AS "allowance",
        -- Reward Amount
        NVL((
          SELECT SUM(r.AMOUNT)
